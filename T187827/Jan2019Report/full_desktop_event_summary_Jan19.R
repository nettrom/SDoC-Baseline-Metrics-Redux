...
p <-  zrr %>%
        ...
        ggplot2::labs(x = NULL, color = "Group", y = "Zero results rate", title = "Proportion of full-text searches on desktop that did not yield any results", subtitle = "With 95% credible intervals. Data from 2018-10-18 to 2019-01-16") +
        wmf::theme_min() 

ggsave("zrr_all_Jan19.png", p, path = fig_path, units = "in", dpi = plot_resolution, height = 6, width = 10, limitsize = FALSE)
rm(p)

#Breakdown by day

p <- searches %>%
        dplyr::filter(date < "2019-01-17") %>% #remove data from last day due to incomplete data on that day.
        group_by(date, wiki) %>%
...
ggsave("daily_zrr_Jan19.png", p, path = fig_path, units = "in", dpi = plot_resolution, height = 6, width = 10, limitsize = FALSE)
rm(p)
...
p <-  ctr %>%
        ...
        ggplot2::scale_y_continuous(labels = scales::percent_format()) +
        ggplot2::scale_color_brewer("Wiki", palette = "Set1") +
        ggplot2::labs(x = NULL, y = "Clickthrough rate", title = "Desktop full-text search results clickthrough rates", subtitle = "With 95% credible intervals. Data from 2018-10-18 to 2019-01-16.") +
        wmf::theme_min()
ggsave("ctr_all_Jan19.png", p, path = fig_path, units = "in", dpi = plot_resolution, height = 6, width = 10, limitsize = FALSE)
rm(p)
...
p <- searches %>%
        filter(date < "2019-01-17") %>% #remove data from last day due to incomplete data on that day.
        keep_where(`got same-wiki results` == TRUE) %>%
        group_by(wiki, date) %>%
        summarize(n_search = n(), clickthroughs = sum(`same-wiki clickthrough`)) %>%
        ungroup %>%
        ...
        wmf::theme_min()
ggsave("daily_ctr_Jan19.png", p, path = fig_path, units = "in", dpi = plot_resolution, height = 6, width = 10, limitsize = FALSE)
rm(p)

#Broken down by type of search page activity
p <- searches %>%
  filter(date <= '2019-01-14')%>% #remove last few days due to incomplete data
  group_by(date) %>%
  summarize( 
    `All searches` = n(),
...
ggsave("daily_search_activity_Jan19.png", p, path = fig_path, units = "in", dpi = plot_resolution, height = 6, width = 10, limitsize = FALSE)
rm(p)
...
p <- offset_prop %>%
        ...
        ggplot2::scale_y_continuous(labels = scales::percent_format()) +
        ggplot2::scale_color_brewer("Wiki", palette = "Set1") +
        ggplot2::labs(x = NULL, y = "Proportion of searches", title = "Proportion of desktop full-text searches with clicks to see other pages of the search results", subtitle = "With 95% credible intervals. Data from 2018-10-18 to 2019-01-16.") +
        wmf::theme_min(plot.title = element_text(size=14))
ggsave("serp_offset_all_Jan19.png", p, path = fig_path, units = "in", dpi = plot_resolution, height = 6, width = 10, limitsize = FALSE)
rm(p)
...
ggsave("daily_serp_offset_Jan19.png", p, path = fig_path, units = "in", dpi = plot_resolution, height = 6, width = 10, limitsize = FALSE)
rm(p)
...
ggsave("survival_visitedPages_all_Jan19.png", p, path = fig_path, units = "in", dpi = plot_resolution, height = 6, width = 10, limitsize = FALSE)
rm(p)
...
p <- dwell_time_bycheckin %>%
        ...
        labs(
                ...
                subtitle = "Data from 2018-10-18 to 2019-01-16",
                y = "Proportion of visits.")
   

ggsave("dwell_time_bycheckin_Jan19.png", p, path = fig_path, units = "in", dpi = plot_resolution, height = 6, width = 10, limitsize = FALSE)
rm(p)
