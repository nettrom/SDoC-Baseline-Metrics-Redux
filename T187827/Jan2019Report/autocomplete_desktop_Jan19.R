...
save(autocomplete_events_raw, file = "data/autocomplete_events_raw_enwiki_commons_Jan19.RData")

#Load and clean data (convert timestamp, remove duplicated event, order)
load("data/autocomplete_events_raw_enwiki_commons_Jan19.RData")
...
# Remove outliers with 1000 or more searches
...
p <-  ctr_raw %>%
        ...
        ggplot2::labs(x = NULL, y = "Clickthrough rate", title = "Autocomplete search clickthrough rates on desktop", 
                      subtitle = "With 95% credible intervals. Data from 2018-10-18 to 2019-01-16",
                      caption = "*clickthrough rates = total clicks / total search result pages.") +
        wmf::theme_min()
ggsave("autocomplete_all_ctr_Jan19.png", p, path = fig_path, units = "in", dpi = plot_resolution, height = 6, width = 10, limitsize = FALSE)
...
p <- autocomplete_events %>%
        filter(date < "20190110") %>% #remove data from last days due to incomplete data on that day and outlier
        ...
        labs(title = "Daily autocomplete search clickthrough rates on desktop", subtitle = "Dashed line marks the overall clickthrough rate.",
             caption = "*clickthrough rates = total clicks / total search result pages.") +
        wmf::theme_min()
ggsave("daily_autocomplete_ctr_Jan19.png", p, path = fig_path, units = "in", dpi = plot_resolution, height = 6, width = 10, limitsize = FALSE)
rm(p)
