...
p <- monthly_deletions %>%
  ...
  tally %>%
        ...
        labs(
                ...
                x= "Date",
                y = "Proportion",
                title = "Proportion of all deleted files deleted within 1 month (28 days)",
...
p <- filter(monthly_deletions, !media_type %in% c("unknown", "archive", "text")) %>% 
        ...
        labs(title = "Proportion of deleted files deleted within 1 month (28 days) by media type", 
             fill = "Deleted",
             x = NULL,
             y = "Proportion of deleted files deleted within 1 month ",
        subtitle = "Includes only files deleted as of 2018-02-01")
...
## Proportion of newly uploaded files deleted within a month of upload (Looking at all uploaded file each month [deleted and not deleted as of 2018-02-01])
...
monthly_deletions_all$uploads[is.na(monthly_deletions_all$uploads)] <- 0
```

```{r}
monthly_deletions_alluploads <- monthly_deletions_all %>%
        filter(upload_date < "2018-02-01") %>%
        mutate(upload_month = floor_date(upload_date, "month")) %>%
...
```{r}
#Determine the months with the highest number of uploaded files deleted within a month.
...
```{r}
p <- ggplot(monthly_deletions_alluploads,
...
```{r}
mean(monthly_deletions_alluploads$prop_deleted)
```
